...
do
  ...
  curl -X PUT "http://hr-ltx1.${zk_env}.linkedin.com:12954/admin/v2/namespaces/${zk_cluster}/clusters/${cluster_name}"
  sleep 5
  json=" { \"id\" : \"${cluster_name}\", \"simpleFields\" : { \"allowParticipantAutoJoin\" : \"true\"} } "
  curl -X POST -H "Content-Type: application/json" "hr-ltx1.${zk_env}.linkedin.com:12954/admin/v2/namespaces/${zk_cluster}/clusters/${cluster_name}/configs?command=update" -d "${json}"
  echo "2. Register ${cluster_name}"
  curl -X POST "http://hr-ltx1.${zk_env}.linkedin.com:12954/admin/v2/namespaces/${zk_cluster}/clusters/${cluster_name}?command=activate&superCluster=${super_cluster}"
  curl "hr-ltx1.${zk_env}.linkedin.com:12954/admin/v2/namespaces/${zk_cluster}/clusters/${cluster_name}/configs"
done
