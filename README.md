...
- code-review tooling in `bin/`: `ctxpack` assembles a local review context pack
  ...
  24 hours after its owner ends is hibernated and remains restorable. Design notes in
  `llm/context-packs/README.md`; the corpora they read are
  private and live outside this repository (see that doc's Boundary section)
- desktop-app glue in `bin/`: `t3-claude` hands the T3 Code desktop app a
  terminal-equivalent environment before exec'ing the `claude` its PATH resolves
  to, so GUI-spawned threads keep the same launcher, node runtime, MCP servers,
  and skills a terminal session gets. Point the app's Claude binary path at it
  (`~/.t3/userdata/settings.json` -> `providers.claudeAgent.binaryPath`), and use
  `t3-claude --t3-env` to see what it resolved. It answers the app's `--version`
  health check from the installed CLI, which a launcher that resolves versions
  over the network cannot do inside the app's 4s probe budget. Flags for a
  private launcher belong in a private wrapper reached through `T3_CLAUDE_BIN`;
  the app appends its `launchArgs` after the CLI's own arguments, where a
  launcher's own options are no longer recognized
...
| --- | --- | --- | --- |
| Shared base | `fun-bash-automations` | `llm/`, `codex/`, `zsh/personal.zsh`, `git/`, `bin/` | `~/.claude`, `~/.codex`, `~/.cursor/skills`, `~/.zsh/personal.zsh`, `~/.config/git/ignore`, `~/.local/bin` |
...
bin/fba-deploy --cursor-only
```

`fba-deploy` renders shared config into `~/.claude` and `~/.codex`, and shared
skills into `~/.cursor/skills` when Cursor is installed. Private installers can
pass `--external-skills-dir <dir>` to project an explicit overlay into every
selected runtime; a sibling `<dir>.allowlist` limits that overlay and is also
used to prune retired entries. `fba-deploy` does not discover private overlays
implicitly or re-enroll Git hook trust. Hook enrollment is a separate,
explicitly audited operation described under Delivery Policy.
