...
Use the repo, branch, cwd, task, open PR, and recent commits together. If the task asks about PRs, commits, or branch work and an open PR is provided, prefer the PR title as the source of context. If the branch is broad or not meaningful (examples: main, master, dev), do not use the branch name as context. Do not use generic repo names like fun-bash-automations as context when a topic such as Agent notifications, Hook runtime, Stack tooling, or Push-gate leases is available.
