...
while :; do
  ...
  resp=$(alerter --title "$TITLE" --subtitle "$display_subtitle" --message "$summary" \
    --ignore-dnd --actions Respond --timeout "$(alert_timeout)" --sound Pop \
    ${GROUP:+--group "$GROUP"} \
  ...
  if [[ "$act" = "contentsClicked" || "$act" = "actionClicked" ]] && [ -n "$OPEN_URL" ]; then
    ...
    if command -v notify_click_open_url >/dev/null 2>&1; then
      notify_click_open_url "input_summary" "Respond" "$act" "$GROUP" "$STATE_ID" "$OPEN_URL" "$notify_started_ms" "$notify_clicked_ms" "$CWD"
    else
