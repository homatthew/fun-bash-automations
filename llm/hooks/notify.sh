...
VSCODE_EXT_PUBLISHER="${OSC_NOTIFIER_PUBLISHER:-wenbopan}"
VSCODE_EXT_NAME="${OSC_NOTIFIER_NAME:-vscode-terminal-osc-notifier}"
VSCODE_EXT_URI_PATH="${OSC_NOTIFIER_URI_PATH:-/focus}"
...
prompt_task_context() {
  ...
  if printf '%s %s' "$REPO" "$clean" | grep -Eiq 'notif|alerter|hook|focus|icon|show|vscode|claude|codex'; then
    ...
    printf 'Docs'
  elif [ -n "${BRANCH:-}" ] && ! printf '%s' "$BRANCH" | grep -Eq '^(main|master|develop|dev)$'; then
...
branch_is_display_useful() {
  local branch="$1"
  [ -n "$branch" ] || return 1
  ! printf '%s' "$branch" | grep -Eq '^(main|master|develop|dev)$'
}
...
dispatch_alerter() {
  ...
  jq -n \
    ...
    > "$spec"
  label="dev.fun-bash-automations.notify.${RUNTIME:-agent}.$$.$RANDOM"
...
dispatch_working_summary() {
  ...
  jq -n \
    ...
    > "$spec"
  label="dev.fun-bash-automations.notify.summary.${RUNTIME:-agent}.$$.$RANDOM"
...
dispatch_final_summary() {
  ...
  jq -n \
    ...
    > "$spec"
  label="dev.fun-bash-automations.notify.final.${RUNTIME:-agent}.$$.$RANDOM"
...
dispatch_input_summary() {
  ...
  jq -n \
    ...
    '{source_text:$source_text, task_summary:$task_summary, title:$title, subtitle:$subtitle, group:$group, sender:$sender, open_url:$open_url, cwd:$cwd, log:$log, state_file:$state_file, state_id:$state_id, summary_file:$summary_file, context_file:$context_file, repo:$repo, branch:$branch, pr_context:$pr_context, recent_commits:$recent_commits}' \
    > "$spec"
  label="dev.fun-bash-automations.notify.input.${RUNTIME:-agent}.$$.$RANDOM"
...
if [ "$RUNTIME" = "codex" ]; then
  ...
  SENDER_BUNDLE="${CLAUDE_NOTIFY_BUNDLE_ID:-dev.fun-bash-automations.claude-notify}"
fi
