...
while IFS= read -r -d '' file; do
  ...
  dest_name=
  case "$lower" in
    jpg|jpeg|png|tif|tiff) dest_name="$stem.jpg" ;;
    avi|mov|mp4|m4v) dest_name="$stem.mp4" ;;
  esac
  [ -n "$dest_name" ] || continue
  if ! mkdir "$claims/$dest_name" 2>/dev/null; then
    rm -rf "$stage"
    echo "Multiple inputs normalize to the same output: $dest_name" >&2
    exit 1
  fi
...
first_output=$(find "$stage" -maxdepth 1 -type f -print -quit)
