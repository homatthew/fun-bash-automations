...
- Second-brain topics: `${SECOND_BRAIN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/second-brain}/topics/` — glob for keywords from the PR when configured
...
3. Send each target to a fresh local reviewer when possible. Prefer a new
   `claude -p` invocation with read-only tools and the strongest supported
