...
# Resolve the diff base: explicit flag, else merge-base against the default
# branch, else HEAD if there are staged/unstaged changes.
resolve_base() {
  local want="${1:-}"
...
# stderr is dropped: in a repo with no commits every form of this is a fatal
# "ambiguous argument HEAD", and the empty result is already the right answer.
changed_files() {
  local base="$1"
  if [ "$base" = "HEAD" ]; then
  ...
  # Untracked files are part of the working snapshot reviewers and tests see.
  # Omitting them can turn a non-landable patch into an apparently green review
  # when tracked code imports an untracked implementation.
...
untracked_diff() {
  ...
  while IFS= read -r f; do
    [ -n "$f" ] || continue
    # Plain `--no-index` emits a compact "Binary files differ" marker for
    # binaries rather than embedding their bytes. Text files are included in
    # full; ctxreview's existing prompt-size cap rejects an oversized result
...
# Only multi-word program identifiers count as convention signal:
# camelCase, multi-word PascalCase, or SCREAMING_SNAKE.
#
# The first cut of this used "any word >=6 chars" and the top hits were
# `without`, `storage`, `status`, `require` — English prose, scored 3/3.
# A convention is something a programmer *named*, and in every language here
# that means a case transition or an underscore.
IDENT_RE='([a-z][A-Za-z0-9]*[A-Z][A-Za-z0-9]*|[A-Z][a-z0-9]+[A-Z][A-Za-z0-9]*|[A-Z][A-Z0-9]*_[A-Z0-9_]+)'

# Precedent only means something inside one language's idiom. Prose files have
# no call sites, so counting tokens across them measures vocabulary, not
# convention.
is_code_path() {
  case "${1##*.}" in
...
section_claim() {
  ...
  if [ "$base" = "HEAD" ]; then
    ...
    # Subjects only. Bodies are frequently longer than the diff and would
    # crowd out every other section of the pack.
...
section_scope() {
  ...
  # Tier trigger mirrors llm/AGENTS.md "Gate Selection". Stated, not enforced.
  ...
  # awk rather than `| head`: head closes the pipe early, which under
  # `set -o pipefail` turns a successful listing into a failed section.
...
section_constitution() {
  ...
  # Nested AGENTS.md: a module-level file overrides the root for its subtree.
  while IFS= read -r f; do
...
# The sibling-precedent miner. This is the section that earns the pack.
#
# Two questions, both answered by counting, not by taste:
#   absent-precedent — N of M sibling files use token X; this file does not.
#   novel token      — this diff introduces X, which 0 siblings use.
#
# The first catches "every other axios factory attaches the auth interceptor".
# The second catches a new pattern being smuggled in next to an old one.
section_precedent() {
  local base="$1"; shift
  local files=("$@")
  ...
  for f in "${files[@]}"; do
    ...
    # Count, per token, how many siblings mention it.
    : > "$tmp/sibtokens"
    local s
    ...
    # Novel tokens ranked by how often the diff actually uses them — an
    # alphabetical head() just returns whatever starts with "A".
    local novel
...
section_history() {
  ...
  for f in "${files[@]}"; do
    [ -f "$f" ] || continue
    n=$((n+1)); [ "$n" -gt 20 ] && { printf -- '- … (%d more files not shown)\n' $(( ${#files[@]} - 20 )); break; }
    printf '### `%s`\n\n```\n' "$f"
...
# Review lenses: reusable checks distilled from engineers whose work is worth
# learning from, attributed to evidence but named after the principle. These
# are what to *look for*; the bible is what has already been *decided*.
section_lens() {
  local files=("$@")
